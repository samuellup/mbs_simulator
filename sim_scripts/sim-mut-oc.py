...
for chromosome in contigs:
	...
	# Extract M2 mutation positions from info_all_mutations.txt file (chromatide 1), add them to a list, and finally sort the list
	all_mut_pos = list() # Will store the list of mutated positions

	with open("u_data/m2/info_all_mutations.txt") as muts:
		for line in muts:
			sp_mut = line.split()
			all_mut_pos.append(int(sp_mut[1]))
